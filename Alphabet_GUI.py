...
the alphabets using a Pre-Trained CNN model.
...
# loading the model
model = load_model(r'alpharecognition.h5')
...
def predict_digit(filename):
    ...
    for cnt in contours:
        ...
        # Resizing that digit to (192, 192)
        resized_digit = cv2.resize(digit, (192, 192))

        # Padding the digit with 54 pixels of black color (zeros) in each side to 
        #finally produce the image of (300, 300)
        padded_digit = np.pad(resized_digit, ((54, 54), (54, 54)), "constant", constant_values=0)
        im = Image.fromarray(padded_digit)
        im.save('temp.png')
        img = IMG.load_img('temp.png', target_size=(300, 300))
        img = IMG.img_to_array(img)
        img = np.expand_dims(img, axis=0)
        img = np.vstack([img])
        # sending the padded digit to model and prediciting
        
        prediction = model.predict(img/255,batch_size=10)
        #writing prediciton on image
        data = "Pred = " + str(chr(np.argmax(prediction)+65))
        #print(data)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Handwritten Alphabet Recognition GUI")
